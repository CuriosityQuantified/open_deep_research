...
# We use custom model initialization throughout the agent
...
async def clarify_with_user(state: AgentState, config: RunnableConfig) -> Command[Literal["write_research_brief", "__end__"]]:
    ...
    model = init_custom_model(config, model_type="research").with_structured_output(ClarifyWithUser).with_retry(stop_after_attempt=configurable.max_structured_output_retries)
...
async def write_research_brief(state: AgentState, config: RunnableConfig)-> Command[Literal["research_supervisor"]]:
    configurable = Configuration.from_runnable_config(config)
    research_model = init_custom_model(config, model_type="research").with_structured_output(ResearchQuestion).with_retry(stop_after_attempt=configurable.max_structured_output_retries)
...
async def supervisor(state: SupervisorState, config: RunnableConfig) -> Command[Literal["supervisor_tools"]]:
    ...
    research_model = init_custom_model(config, model_type="research").bind_tools(lead_researcher_tools).with_retry(stop_after_attempt=configurable.max_structured_output_retries)
...
async def researcher(state: ResearcherState, config: RunnableConfig) -> Command[Literal["researcher_tools"]]:
    ...
    research_model = init_custom_model(config, model_type="research").bind_tools(tools).with_retry(stop_after_attempt=configurable.max_structured_output_retries)
...
async def compress_research(state: ResearcherState, config: RunnableConfig):
    ...
    synthesizer_model = init_custom_model(config, model_type="compression")
    researcher_messages = state.get("researcher_messages", [])
...
async def final_report_generation(state: AgentState, config: RunnableConfig):
    ...
    configurable = Configuration.from_runnable_config(config)
    writer_model = init_custom_model(config, model_type="final_report")
    ...
    while current_retry <= max_retries:
        ...
        try:
            ...
            if is_token_limit_exceeded(e, configurable.final_report_model_name):
                if current_retry == 0:
                    model_token_limit = get_model_token_limit(configurable.final_report_model_name)
