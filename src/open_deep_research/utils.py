...
async def tavily_search(
    ...
    summarization_model = init_custom_model(config, model_type="summarization").with_structured_output(Summary).with_retry(stop_after_attempt=configurable.max_structured_output_retries)
    async def noop():
        return None
    summarization_tasks = [
...
# Custom Model Utils
##########################
def init_custom_model(config: RunnableConfig, model_type: str = "research", max_tokens: Optional[int] = None) -> BaseChatModel:
    """
    Initialize a custom model using ChatOpenAI wrapper with custom endpoint.
    
    Args:
        config: Runnable configuration
        model_type: Type of model to initialize ("summarization", "research", "compression", "final_report")
        max_tokens: Override max tokens (if None, uses configured value)
    """
    configurable = Configuration.from_runnable_config(config)
    
    # Map model type to configuration fields
    model_config_map = {
        "summarization": {
            "name": configurable.summarization_model_name,
            "base_url": configurable.summarization_model_base_url,
            "api_key": configurable.summarization_model_api_key,
            "max_tokens": configurable.summarization_model_max_tokens
        },
        "research": {
            "name": configurable.research_model_name,
            "base_url": configurable.research_model_base_url,
            "api_key": configurable.research_model_api_key,
            "max_tokens": configurable.research_model_max_tokens
        },
        "compression": {
            "name": configurable.compression_model_name,
            "base_url": configurable.compression_model_base_url,
            "api_key": configurable.compression_model_api_key,
            "max_tokens": configurable.compression_model_max_tokens
        },
        "final_report": {
            "name": configurable.final_report_model_name,
            "base_url": configurable.final_report_model_base_url,
            "api_key": configurable.final_report_model_api_key,
            "max_tokens": configurable.final_report_model_max_tokens
        }
    }
    
    if model_type not in model_config_map:
        raise ValueError(f"Invalid model type: {model_type}. Must be one of: {list(model_config_map.keys())}")
    
    model_config = model_config_map[model_type]
    
    # Check if configuration is complete
    if not model_config["name"] or not model_config["base_url"] or not model_config["api_key"]:
        raise ValueError(f"{model_type.title()} model configuration is incomplete. Please provide model_name, base_url, and api_key.")
    
    # Use provided max_tokens or fall back to configured value
...
def get_api_key_for_model(model_name: str, config: RunnableConfig):
    ...
    # since we're using custom model configuration
    return None
